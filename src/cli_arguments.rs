...
use shlex::Shlex;
use skim::{
    CaseMatching, FuzzyAlgorithm, RankCriteria, Selector, SkimOptions, prelude::DefaultSkimSelector, tui::options::{PreviewLayout, TuiLayout}
};
...
impl CliArguments {
    ...
    pub fn new(call: &EvaluatedCall, engine: &EngineInterface) -> Result<Self, LabeledError> {
        ...
        Ok(Self {
            ...
            layout: call.get_flag("layout")?.or(env_defaults.layout).map(|layout| TuiLayout::from_str(&layout, true)).transpose().map_err(|err| {
    ...
    pub fn to_skim_options(&self) -> SkimOptions {
        ...
        SkimOptions {
            ...
            preview_window: preview_window.as_deref().map(PreviewLayout::from).unwrap_or_default(),
            reverse: *reverse,
